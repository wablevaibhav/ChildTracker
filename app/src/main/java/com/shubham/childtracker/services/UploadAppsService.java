...
public class UploadAppsService extends JobService {
    ...
    private void uploadApps(JobParameters params) {
        ...
        new Thread(this::getRecentAppStats).start();
            ...
            for (ApplicationInfo applicationInfo : applicationInfoList) {
                for (App app : apps) {
                    if (app.getPackageName().equals(applicationInfo.packageName)) {
                        appsList.add(new App((String) applicationInfo.loadLabel(packageManager), applicationInfo.packageName, app.isBlocked()));
    ...
    private void getInstalledApplication() {
        ...
        applicationInfoList.sort(new ApplicationInfo.DisplayNameComparator(packageManager));
