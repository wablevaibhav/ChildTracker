...
import com.shubham.childtracker.Activities.LoginActivity;
...
public class AccountUtils {
    ...
    public static void changePassword(final Context context, String newPassword) {
        Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).updatePassword(newPassword).addOnCompleteListener(task -> {
    ...
    public static void deleteAccount(Context context, String password) {
        final String providerId = Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getProviderId();
    ...
    private static void deleteAccountData(final String providerId, final String password, final Context context) {
        ...
        Query parentQuery = databaseReference.child("parents").orderByChild("email").equalTo(Objects.requireNonNull(user).getEmail());
        parentQuery.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    String imgUrl = Objects.requireNonNull(parent).getProfileImage();
                    ...
                    childQuery.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            if (dataSnapshot.exists()) {
                                ...
                                String imgUrl = Objects.requireNonNull(child).getProfileImage();
                                removeImage(imgUrl, providerId, password, context);
    ...
    private static void removeImage(String imgUrl, final String providerId, final String password, final Context context) {
        if (imgUrl.contains("https://firebasestorage.googleapis.com")) {
            ...
            profileImageStorageReference.delete()
                    .addOnSuccessListener(aVoid -> deleteUser(providerId, password, context))
    ...
    private static void deleteUser(String providerId, final String password, final Context context) {
        ...
        if (providerId.equals("google.com"))
            credential = GoogleAuthProvider.getCredential(Objects.requireNonNull(user).getEmail(), null);

        else
            credential = EmailAuthProvider.getCredential(Objects.requireNonNull(Objects.requireNonNull(user).getEmail()), password);
